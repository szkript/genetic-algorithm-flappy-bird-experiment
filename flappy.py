...
logging.basicConfig(level=logging.INFO, filename="logs/generations_{0}.log".format(total_models))
logging.info("population :" + str(total_models))
...
for i in range(total_models):
    model = Sequential()
    model.add(Dense(7, kernel_initializer='uniform', activation='sigmoid', input_dim=3))
...
def showGameOverScreen(crashInfo):
    ...
    for select in range(total_models):
        ...
        if fitness[select] > fittest_model["fitness"]:
            fittest_model["id"] = select
            fittest_model["fitness"] = fitness[select]
    for select in range(total_models):
    ...
    info_string = "gen: " + str(generation) + " ,score: " + str(current_score) + " ,high score: " + str(
        high_score) + " ,fittest: " + str(fittest_model["id"]) + " ,fitness: " + str(fittest_model["fitness"])
